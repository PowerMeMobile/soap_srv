    ...
    case authenticate(CustomerID, UserName, Password) of
        {ok, Customer} ->
            Originators = [Addr#addr.addr ||
                Addr <- Customer#auth_customer_v3.originators],
            Credits = credit_left(Customer#auth_customer_v3.pay_type,
                                  Customer#auth_customer_v3.credits),
            {ok, #'AuthResult'{
                'Result' = ?E_SUCCESS,
                'NetPoints' = Credits,
...
handle(check_inbox_activated, Req = #'InboxProcessing'{}, Customer) ->
    case lists:keyfind(<<"inbox">>, #feature_v1.name, Customer#auth_customer_v3.features) of
...
handle(inbox_processing, Req = #'InboxProcessing'{user = User}, Customer) ->
    ...
    Credits = credit_left(Customer#auth_customer_v3.pay_type,
                          Customer#auth_customer_v3.credits),
    inbox_processing(CustomerUuid, UserId, Operation, MsgIds, Credits);
...
send_result(#send_result{
    ...
    {ok, #'SendResult'{
        ...
        'TransactionID' = ReqId,
        'NetPoints' = credit_left(Customer#auth_customer_v3.pay_type, CreditLeft)
    }};
...
inbox_processing(CustomerUuid, UserId, Operation, MsgIds, Credits) ->
    case alley_services_api:process_inbox(CustomerUuid, UserId,
        ...
        {ok, #inbox_resp_v1{result = Result}} ->
            handle_inbox_response(Result, Credits);
        {error, Error} ->
            handle_inbox_error_response(Operation, Error)
    end.
...
handle_inbox_response({info, Info}, Credits) ->
    ...
    "<inboxinfo xmlns=\"\">",
    "<result>OK</result>",
    "<credits>", Credits/binary, "</credits>",
...
handle_inbox_response({messages, Messages}, Credits) ->
    Result =
    <<
    "<inboxlist xmlns=\"\">",
    "<result>OK</result>",
...
handle_inbox_response({deleted, Deleted}, Credits) ->
    Result =
    <<
    "<inboxdel xmlns=\"\">",
    "<result>OK</result>"
